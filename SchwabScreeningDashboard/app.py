...
# ── Duddy's Badass Screener state ─────────────────────────────────────────

_badass_state: dict = {
    "running": False, "started": None, "completed": None,
    "progress": 0, "total": 0, "current": "",
    "results": None, "red_flag": None, "error": None,
    "min_price": 10.0, "min_volume": 500_000, "apply_rf_penalty": True,
}
_badass_lock = threading.Lock()


def _run_badass_background():
    """Background thread for Duddy's Badass Screener."""
    try:
        import badass_screener as bs
        from momentum import fetch_history
        import universe as univ

        symbols, _ = univ.load_universe()
        with _badass_lock:
            _badass_state["total"] = len(symbols)
            min_price       = _badass_state["min_price"]
            min_volume      = _badass_state["min_volume"]
            apply_rf_penalty = _badass_state["apply_rf_penalty"]

        def cb(i, total, ticker):
            with _badass_lock:
                _badass_state["progress"] = i
                _badass_state["current"]  = ticker
        ...
        rows, red_flag = bs.run_badass_screen(
            ...
            apply_rf_penalty=apply_rf_penalty,
            progress_cb=cb,
        )

        with _badass_lock:
            _badass_state["results"]   = rows
            _badass_state["red_flag"]  = red_flag
            _badass_state["completed"] = datetime.now()

        from scan_cache import save_scan
        save_scan("badass", {"rows": rows, "red_flag": red_flag})
        logger.info("Badass screen completed: %d stocks qualified", len(rows))

    except Exception as e:
        logger.exception("Badass scan failed: %s", e)
        with _badass_lock:
            _badass_state["error"] = str(e)
    finally:
        with _badass_lock:
            _badass_state["running"] = False


@app.route("/badass")
def badass_page():
    """Duddy's Badass Stock Screener page."""
    with _badass_lock:
        running   = _badass_state["running"]
        completed = _badass_state["completed"]
        rows      = _badass_state.get("results")
        red_flag  = _badass_state.get("red_flag") or {}
        error     = _badass_state.get("error")
    ...
    display_rows = rows or []
    if filter_action == "strong":
        display_rows = [r for r in display_rows if r.get("action") == "Strong Buy"]
    elif filter_action == "buy":
        display_rows = [r for r in display_rows if r.get("action") in ("Strong Buy", "Buy")]
    elif filter_action == "watch":
        display_rows = [r for r in display_rows if r.get("action") in ("Strong Buy", "Buy", "Watch")]
    ...
    if rows:
        summary = {
            "total_scanned": _badass_state.get("total", len(rows)),
            "qualified":     len(rows),
            "strong_buy":    sum(1 for r in rows if r.get("action") == "Strong Buy"),
            "buy":           sum(1 for r in rows if r.get("action") == "Buy"),
            "watch":         sum(1 for r in rows if r.get("action") == "Watch"),
            "completed":     completed.strftime("%Y-%m-%d %H:%M:%S") if completed else None,
        }
    ...
    return render_template(
        "badass.html",
        running=running, rows=display_rows, summary=summary,
        red_flag=red_flag, error=error,
        filter_action=filter_action, top_n=top_n,
    )


@app.route("/badass/start", methods=["POST"])
def badass_start():
    data = request.get_json(silent=True) or {}
    min_price       = float(data.get("min_price",       10.0))
    min_volume      = int(data.get("min_volume",         500_000))
    apply_rf_penalty = bool(data.get("apply_rf_penalty", True))

    with _badass_lock:
        if _badass_state["running"]:
            return jsonify({"status": "already_running"})
        _badass_state.update({
            "running": True, "started": datetime.now(),
            "completed": None, "progress": 0, "total": 0,
            "current": "", "results": None, "error": None,
    ...
    threading.Thread(target=_run_badass_background, daemon=True).start()
    return jsonify({"status": "started"})


@app.route("/badass/status")
def badass_status():
    with _badass_lock:
        return jsonify({
            "running":   _badass_state["running"],
            "progress":  _badass_state["progress"],
            "total":     _badass_state["total"],
            "current":   _badass_state["current"],
            "completed": _badass_state["completed"].isoformat() if _badass_state["completed"] else None,
            "error":     _badass_state["error"],
        })


@app.route("/badass/export/csv")
def badass_export_csv():
    with _badass_lock:
        rows = _badass_state.get("results")
    if not rows:
        return "No results to export", 404
    ...
    return Response(
        csv_data, mimetype="text/csv",
        headers={"Content-Disposition": f"attachment; filename=badass_screen_{ts}.csv"},
    )
...
def _startup_scans():
    ...
    cached_badass = load_scan("badass")
    if cached_badass:
        with _badass_lock:
            _badass_state["results"]   = cached_badass["data"]["rows"]
            _badass_state["red_flag"]  = cached_badass["data"].get("red_flag")
            _badass_state["completed"] = datetime.fromisoformat(cached_badass["timestamp"])
        logger.info("Loaded cached Badass results (%d rows)", len(cached_badass["data"]["rows"]))
